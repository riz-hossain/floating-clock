...
NEAR = {"busy": "Finding the masjids near you", "found": "Found the masjids near you", "none": "Nothing found near you"}
line = Timeline(Clock())
line.plan("masjids near you", [Section("site", "s", "", timeline.WEBSITE)], words=NEAR)
check("a look for masjids near you says so in its own words, with no masjid's name in them",
      view.headline(line.snapshot()) == ("Finding the masjids near you", view.FG), str(view.headline(line.snapshot())))
line.finish("found", "")
check("and in its own words when it is done", view.headline(line.snapshot()) == ("Found the masjids near you", view.GOOD))
line = Timeline(Clock())
line.plan("masjids near you", [Section("site", "s", "", timeline.WEBSITE)], words=NEAR)
line.finish("none", "nothing")
check("and when there were none", view.headline(line.snapshot()) == ("Nothing found near you", view.BAD))
line = Timeline(Clock())
line.plan("masjids near you", [Section("site", "s", "", timeline.WEBSITE)], words=NEAR)
line.say("none", "Could not find where you are")
line.finish("none", "nothing")
check("what it says can be changed once it is known why it failed", view.headline(line.snapshot()) == ("Could not find where you are", view.BAD))
line = Timeline(Clock())
line.plan("masjids near you", [Section("site", "s", "", timeline.WEBSITE)], words=NEAR)
line.finish("stopped", "Stopped.")
check("the words it does not give are the usual ones", view.headline(line.snapshot()) == ("Stopped", view.MUTED))
check("the words travel in the snapshot", line.snapshot()["words"]["busy"] == "Finding the masjids near you", str(line.snapshot()["words"]))
line = Timeline(Clock())
line.plan("Test Masjid", [Section("site", "s", "", timeline.WEBSITE)])
line.say("found", "Found it")
check("a check of a masjid keeps its own words unless told otherwise", view.headline(line.snapshot()) == ("Checking Test Masjid", view.FG))
other = Timeline(Clock())
other.plan("Other Masjid", [Section("site", "s", "", timeline.WEBSITE)])
check("and one timeline's words are not another's", other.snapshot()["words"]["found"] == "Times found" and timeline.CHECK_WORDS["found"] == "Times found")
try:
    said = (view.headline({"title": "Old Masjid", "outcome": ""}), view.headline({"title": "Old Masjid", "outcome": "found"}))
except Exception as exc:                     # noqa: BLE001 -- a snapshot without words must not need them
    said = "raised %s" % exc.__class__.__name__
check("a snapshot from before there were words is worded as it always was",
      said == (("Checking Old Masjid", view.FG), ("Times found", view.GOOD)), str(said))
timeline.current().say("none", "nobody is listening")
timeline.current().plan("x", [], words=NEAR)
check("and nobody listening accepts them, and keeps nothing", timeline.current().snapshot()["rows"] == [])
